...
setwd("C:/Users/Mason Kulbaba/Dropbox/Rscripts/chamaecrista-adaptive-capacity/bootstrap/KW/2017")
...
VaW<- function (rout2017){
  ...
  map <- function(b) {
    ...
    hoom.star<- matrix(hoom.star, ncol = 5)
    return(hoom.star[2302, 5]) # return value of 5th node (seed #) for 2302nd indiv (1st indiv in block 6). Change this to whatever your terminal fitness node is (e.g. fruit #)
  }
  fred<- Vectorize(map)
...
save.image("doubleboot_KW2017_run.RData")

# Generate Va(W) from data
KW_Va.hat <- VaW(rout2017) # generates Va(W) from data
...
KW_Va.hat - rev(crit) * se.KW_Va.hat_sd