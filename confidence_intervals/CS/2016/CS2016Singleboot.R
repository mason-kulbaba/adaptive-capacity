...
VaW<- function (rout2016){
  ...
  map <- function(b) {
    ...
    alpha[7] <- alpha[7] + b # adding fixed effect for fit:block6 (e.g. alpha[11]), make sure this matches your model
    hoom.star <- predict(rout2016$obj, newcoef = alpha)
    hoom.star<- matrix(hoom.star, ncol = 5)
    return(hoom.star[251, 5]) # return value of 5th node (seed #) for 251st indiv (1st indiv in block 6). Change this to whatever your terminal fitness node is (e.g. fruit #)
  }
  fred<- Vectorize(map)
  ...
  CS_SE<- 4 * moom.star[251, 5]^2 * soutstar$nu["parental", "Std. Error"] / map(0) 
...
#save.image("singleboot_CS2016_run.RData")

load("singleboot_CS2016_run.RData")

# Generate Va(W) from data
CS_Va.hat <- VaW(rout2016)[1] # generates va from data
CS_Va.hat_sd <- VaW(rout2016)[2] # using the empirical sd.
