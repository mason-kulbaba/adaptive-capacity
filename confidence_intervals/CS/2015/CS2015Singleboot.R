...
VaW<- function (rout2015){
  ...
  map <- function(b) {
    ...
    alpha[6] <- alpha[6] + b # adding fixed effect for fit:block6 (e.g. alpha[11]), make sure this matches your model
    hoom.star <- predict(rout2015$obj, newcoef = alpha)
    hoom.star<- matrix(hoom.star, ncol = 5)
    return(hoom.star[1, 5]) # return value of 5th node (seed #) for 2274th indiv (1st indiv in block 6). Change this to whatever your terminal fitness node is (e.g. fruit #)
  }
  fred<- Vectorize(map)
  ...
  CS_SE<- 4 * moom.star[1, 5]^2 * soutstar$nu["parental", "Std. Error"] / map(0) 
...
#save.image("singleboot_CS2015_run.RData")

load("singleboot_CS2015_run.RData")

#Removetwo "-1" direction of recession

CS_Va.star<-subset(CS_Va.star, CS_Va.star>0)

se.CS_Va.star_sd<-subset(se.CS_Va.star_sd, se.CS_Va.star_sd>0)

# Generate Va(W) from data
CS_Va.hat <- VaW(rout2015)[1] # generates va from data
CS_Va.hat_sd <- VaW(rout2015)[2] # using the empirical sd.
