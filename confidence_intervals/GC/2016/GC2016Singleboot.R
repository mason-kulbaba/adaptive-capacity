...
VaW<- function (rout2016){
  ...
  GC_SE<- 4 * moom.star[2274, 5]^2 * soutstar$nu["parental", "Std. Error"] / map(0) 
...
#save.image("singleboot_GC2016_run.RData")

load("singleboot_GC2016_run.RData")

# Generate Va(W) from data
GC_Va.hat <- VaW(rout2016)[1] # generates va from data
GC_Va.hat_sd <- VaW(rout2016)[2] # using the empirical sd.
