...
VaW<- function (rout2017){
  ...
  map <- function(b) {
    ...
    alpha[12] <- alpha[12] + b # adding fixed effect for fit:block6 (e.g. alpha[11]), make sure this matches your model
    hoom.star <- predict(rout2017$obj, newcoef = alpha)
    hoom.star<- matrix(hoom.star, ncol = 5)
    return(hoom.star[3126, 5]) # return value of 5th node (seed #) for 2274th indiv (1st indiv in block 6). Change this to whatever your terminal fitness node is (e.g. fruit #)
  }
  fred<- Vectorize(map)
  ...
  GC_SE<- 4 * moom.star[3126, 5]^2 * soutstar$nu["parental", "Std. Error"] / map(0) 
...
#save.image("singleboot_GC2017_run.RData")

load("singleboot_GC2017_run.RData")

# Generate Va(W) from data
GC_Va.hat <- VaW(rout2017)[1] # generates va from data
GC_Va.hat_sd <- VaW(rout2017)[2] # using the empirical sd.
