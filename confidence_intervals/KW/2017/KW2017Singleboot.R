...
VaW<- function (rout2017){
  ...
  map <- function(b) {
    ...
    hoom.star<- matrix(hoom.star, ncol = 5)
    return(hoom.star[2302, 5]) # return value of 5th node (seed #) for 2274th indiv (1st indiv in block 6). Change this to whatever your terminal fitness node is (e.g. fruit #)
  }
  fred<- Vectorize(map)
  ...
  KW_Va<- 4*moom.star[2302 , 5]^2 * rout2017$nu[1]/map(0) # final calcuation of additive genetic variance for fitness
  soutstar <- summary(rout2017)
  KW_SE<- 4 * moom.star[2302, 5]^2 * soutstar$nu["parental", "Std. Error"] / map(0) 
...
#save.image("singleboot_KW2017_run.RData")

load("singleboot_KW2017_run.RData")


# Generate Va(W) from data
KW_Va.hat <- VaW(rout2017)[1] # generates va from data
KW_Va.hat_sd <- VaW(rout2017)[2] # using the empirical sd.
