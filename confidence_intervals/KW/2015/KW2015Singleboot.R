...
VaW<- function (rout2015){
  ...
  map <- function(b) {
    ...
    alpha[9] <- alpha[9] + b # adding fixed effect for fit:block6 (e.g. alpha[11]), make sure this matches your model
    hoom.star <- predict(rout2015$obj, newcoef = alpha)
    hoom.star<- matrix(hoom.star, ncol = 5)
    return(hoom.star[1570, 5]) # return value of 5th node (seed #) for 2274th indiv (1st indiv in block 6). Change this to whatever your terminal fitness node is (e.g. fruit #)
  }
  fred<- Vectorize(map)
  ...
  KW_Va<- 4*moom.star[1570 , 5]^2 * rout2015$nu[1]/map(0) # final calcuation of additive genetic variance for fitness
  soutstar <- summary(rout2015)
  KW_SE<- 4 * moom.star[1570, 5]^2 * soutstar$nu["parental", "Std. Error"] / map(0) 
...
#save.image("singleboot_KW2015_run.RData")

load("singleboot_KW2015_run.RData")

#remove 5 cases of "-1" direction of recession error
KW_Va.star<- subset(KW_Va.star, KW_Va.star >0)
se.KW_Va.star_sd<- subset(se.KW_Va.star_sd, se.KW_Va.star_sd>0)


# Generate Va(W) from data
KW_Va.hat <- VaW(rout2015)[1] # generates va from data
KW_Va.hat_sd <- VaW(rout2015)[2] # using the empirical sd.
